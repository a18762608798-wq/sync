...
function get_ssh_spectrum(qubit_num::Int, eigvals::Int, slist, δlist)
    get_H(s, δ) = get_ssh_H(qubit_num, s, δ)
    spectrum, _ = get_spectrum(get_H, eigvals, slist, δlist)
    return spectrum
end

function save_ssh_spectrum(path::String, qubit_num::Int, eigvals::Int, grid_length::Int, param_num::Int)
    ...
    for s_idx in eachindex(param_list)
        ...
        s_spectrum[:, :, s_idx] = get_ssh_spectrum(qubit_num, eigvals, s_grid, grid)
    end
    # δ
    for δ_idx in eachindex(param_list)
        δ_grid = fill(param_list[δ_idx], grid_length)
        δ_spectrum[:, :, δ_idx] = get_ssh_spectrum(qubit_num, eigvals, grid, δ_grid)
    end
    npzwrite(
        path,
        Dict(
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    save_ssh_spectrum(path, 8, 4, 50, 6)
end
