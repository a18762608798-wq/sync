...
function save_ssh_phase(path::String, qubit_num::Int, N::Int; sparse=true)
    ...
    @showprogress Threads.@threads for s_idx = 1:N
        for δ_idx = 1:N
            ZR_vals[s_idx, δ_idx] = get_ssh_ZR(qubit_num, s_grid[s_idx], δ_grid[δ_idx]; sparse=sparse)
        end
    end
    npzwrite(
        path,
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    save_ssh_phase(path, 8, 100; sparse=false)
end
