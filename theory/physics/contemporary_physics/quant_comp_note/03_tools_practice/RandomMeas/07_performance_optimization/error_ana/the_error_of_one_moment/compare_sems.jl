...
function calculate_sems(group_name, N)
    ...
    expect_val, sem = modified_get_expect_shadow(
        adjacent_swap_op, permuted_shadows; compute_sem=true, show_progress=false
    );
...
function compare_diff_size(N_ls)
    ...
    @showprogress desc="Compare diff size..." for N_index in 1:N_num
        N = N_ls[N_index]
        N_str = string(N)
...
function compare_diff_settings(N, settings_num_ls, shots_ls)
    ...
    @showprogress desc="Compare diff settings..." for settings_index in 1:settings_ls_length
