...
**这个自由度不是整体相位自由度，而是算符绕自身旋转的自由度。体现在本征态的相对相
位而非整体。换言之整体相位决定的是定义X, Y, Z的时候的形式，表象变换U的自由度来自
其本征向量的相对相位**

Z到X和Z到Y的两个规范对应表象变换只需要一次旋转的补充。比如z到x只需要绕y旋转。但
是开始的绕z旋转和绕被旋转向量的旋转依然对整个刚体有意义。
