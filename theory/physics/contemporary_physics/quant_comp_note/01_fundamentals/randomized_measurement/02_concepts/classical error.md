...
**If $\hat \theta$ is unbiased**, Viz., $E(\widetilde \theta) = \theta$
<!--just the definition of std-->

$$
error = std(\widetilde \theta)
$$

In this paper, > ([@Elben-etal-2023, p. 3])

## case

In the paper [@Elben-etal-2023]

$$
std(\overline X) = \frac{std(X)}{\sqrt{n}}
$$
For the $\pm 1$ variable, $Var(X) \le 1$ ($Var(X) = E(X^2) - [E(X)]^2$，方差必然
大于0，证明小于1即可), Viz.,
