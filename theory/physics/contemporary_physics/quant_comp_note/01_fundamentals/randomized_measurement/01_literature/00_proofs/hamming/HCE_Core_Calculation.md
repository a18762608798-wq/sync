...
Evidently the coefs is $(-1)^q_{s_1,s_2,...} = (-1)^{\sum_{a\in q} s_a}$ , whose
exponent equal the sum of the strings that are non-trivially hit by Pauli bases,
where q is the set of positions on which the nontrivial Pauli operator acts.

Case: $(-1)^{12}_{s_1s_2s_3} = (-1)^{s_1+s_2}$

**The freedom is enough so we could let
$\varepsilon_{ijk,...} = \varepsilon_{\neg i, \neg j, \neg k, ...}$, Viz.,**
