...
==Evidenly z is indepencent with $\beta$, so the joint probability distribution
of $f(z, \beta)$ could be breaken to $f(z)f(\beta)$==
...
**Accroding to the last step of $R_z$, It still has U(1) symmetry around Z.**
(independent with$\beta$)
...
Inverse solution $\alpha$ and $\beta$, evidently
$f(n) dn = f(z)f(\beta) dz d\beta$
...
**The limitation**: From the satisfaction of the expected relation and the
constraint of z values ​​on δ.
