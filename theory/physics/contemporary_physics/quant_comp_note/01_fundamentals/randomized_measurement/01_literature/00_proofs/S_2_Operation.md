...
After replacing it with $dud\varphi$, the probability density becomes constant
(uniform), which is why uniform sampling can be performed directly.
...
Which could preserving the independence of $U_i$, because $R_X(\delta)$ is
constant.
...
Ref to [S2O_the_Expend_of_R_Delta](./S2O_the_Expend_of_R_Delta.md)
...
Think of the last operation $R_Z(\beta)$, The symmetry of $U_z(1)$ is still
vaild.

Therefore I think the operation of
[S2O_the_Expend_of_R_Delta](./S2O_the_Expend_of_R_Delta.md) is still vaild, but
the condition:
