...
Some possible relevant evidence
[classical error](<../../02_concepts/classical error.md>)
...
Ref to
[Classical_Shadow](shadow/Classical_Shadow.md#the-measurement-axis-is-x-y-z)
...
For a generate est of mechanical quantity, we could not get a strict expression,
but giving a relationship with pauli measurement is simple.
...
Ref to
[Classical_Shadow](shadow/Classical_Shadow.md#the-measurement-axis-is-sigma_n)
...
这里连续的误差较小，猜测是即使相同方差数据，数据更集中的测量样本，估计高阶量的时
候会放大这种集中效果带来的方差.
...
Ref to [Pauli_Scan](pauli/Pauli_Scan.md)
...
The second term is zero because the group of B is certain and the statistical
error disappears.

If we intend to est a **expectation of pauli base of a shot**, then we calculate
the variance simply, assume $E[X_O] = p$
