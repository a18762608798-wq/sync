\makeatletter
\@namedef{ver@mathspec.sty}{9999/12/31} % 阻止加载 mathspec
\makeatother
\documentclass[master]{thesis-uestc}
% 在导言区添加 ↓
\usepackage{amsmath, bm, unicode-math}  % amsmath 必须最先加载[1,4](@ref)
\setmathfont{Latin Modern Math}        % 使用预定义数学字体
\renewcommand{\mathbf}{\symbfup}      % 正体粗体替代方案
\newcommand{\mathbd}{\symbf}           % 斜体粗体替代方案
...
\vec{r}_{i,j}=
\begin{cases}
1,f(\vec{x}^{i};\vec{w})\cdot f(\vec{x}^{j};\vec{w})\geq u(\lambda),\\
0,f(\vec{x}^{i};\vec{w})\cdot f(\vec{x}^{j};\vec{w})< l(\lambda), 1\leq i,j\leq n.\\
f(\vec{x}^{i};\vec{w})\cdot f(\vec{x}^{j};\vec{w}),\text{otherwise},
\end{cases}
\end{equation}
